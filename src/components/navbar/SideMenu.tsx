          ...
          >
            ...
            >
              ...
              <motion.div variants={itemVariants} className="grid grid-cols-2 gap-y-4 text-gray-500 font-medium">
                {["Google Ads", "Meta Ads", "Brand Strategy", "Web Development", "Graphic Design", "SEO Optimization"].map((item) => (
                  <motion.div key={item} whileHover={{ x: 5, color: "#000" }} className="flex items-center gap-2 cursor-pointer text-sm">
            ...
            >
              ...
              <div className="flex gap-2">
                {[Youtube, Instagram, Facebook, Twitter].map((Icon, idx) => (
                  <motion.div key={idx} whileHover={{ y: -3 }} className="w-10 h-10 rounded-xl border border-gray-100 flex items-center justify-center text-gray-600">
                    <Icon size={18} />
                  </motion.div>
                ))}
              </div>
